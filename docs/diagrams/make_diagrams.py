...
def lifecycle():
    ...
    s.group_bg(30, 60, 1100, 210, 'Статусы документа')
    s.pill(60, 130, 140, 'Черновик', VIOLET, '#ECEAFB')
    s.pill(285, 130, 185, 'На согласовании', BLUE, PALE)
    s.pill(560, 130, 170, 'Опубликован', GREEN, '#E6F5EC')
    s.pill(910, 130, 140, 'В архиве', MUTED, '#F6F6F6')
    s.arrow(200, 143, 285, 143, 'Отправить', color=BLUE, marker='arrow')
    s.arrow(470, 143, 560, 143, 'Согласовано', color=GREEN, marker='arrow-green')
    s.path('M 470 155 C 430 185, 240 185, 200 157', color=MUTED, marker='arrow-muted', dashed=True)
    s.text(120, 212, 'Отклонено — на доработку', size=11, color=MUTED, anchor='start')
    s.arrow(730, 143, 910, 143, 'В архив', color=MUTED, marker='arrow-muted')
    s.path('M 910 158 C 820 232, 300 240, 200 162', color=MUTED, marker='arrow-muted', dashed=True)
    s.text(600, 253, 'Снять с публикации / вернуть из архива на доработку', size=11, color=MUTED, anchor='middle')
    s.text(130, 100, 'видят модераторы', size=11, color=MUTED, anchor='middle')
    s.text(377, 100, 'видит согласующий', size=11, color=MUTED, anchor='middle')
    s.text(645, 100, 'видят все сотрудники', size=11, color=MUTED, anchor='middle')
    s.text(980, 100, 'история сохранена', size=11, color=MUTED, anchor='middle')
